...
import { Copy, Pencil, Trash2, Plus, Search, Check, X, Tag, PackageOpen, Save } from 'lucide-react';
...
export default function App() {
  ...
  const [measureVisible, setMeasureVisible] = useState({ 'トップス': true, 'パンツ': true, 'スカート': true });
  ...
  function startEdit(entry) {
    ...
    const prefix = `${fullItemNo(entry.category, entry.itemNo)} ${entry.brand}`.trim();
    const hasPrefix = !!prefix && !!entry.title && entry.title.startsWith(prefix);
    setForm({
      ...
      autoPrefix: hasPrefix,
      appliedPrefix: hasPrefix ? prefix : '',
    });
  }
  ...
  function handleSubmit(e) {
    ...
    if (editingId) {
      const prevEntry = items.find((i) => i.id === editingId);
      const updated = { ...form, id: editingId, createdAt: prevEntry ? prevEntry.createdAt : Date.now() };
      persist(items.map((i) => (i.id === editingId ? updated : i)));
      setEditingId(null);
    } else {
          ...
          {!loaded ? (
            ...
            <div className="flex flex-col gap-3">
              {filtered.map((entry) => {
                ...
                return (
                  <div key={entry.id} className="rounded-xl overflow-hidden" style={{ background: COLORS.surface, border: `1px solid ${COLORS.line}` }}>
                    <div
                    ...
                    >
                      ...
                      <div className="flex items-center gap-1 ml-auto" onClick={(e) => e.stopPropagation()}>
                        <button onClick={() => startEdit(entry)} className="p-1.5 rounded-md hover:opacity-70" style={{ color: COLORS.inkSoft }} title="編集">
                          <Pencil size={15} />
                        </button>
